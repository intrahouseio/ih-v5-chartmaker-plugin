...
module.exports = function(records, readobj) {
  ...
  // Выбрать min/max. Увеличить интервал на 10%, если упор
  if (min == undefined) {
    min = readobj.min || 0;
  } else if (readobj.min == undefined || min <= readobj.min) {  // Если ноль - оставляем 0
    min = min != 0 ? Math.round(min - min * 0.1) : 0;
  } else min = readobj.min;

  // Выбрать min/max. Увеличить интервал на 5%, если упор
  if (max == undefined) {
    max = readobj.max || 1;

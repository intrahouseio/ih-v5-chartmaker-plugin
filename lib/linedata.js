...
module.exports = async function(records, readobj, query, useIds, client, plugin) {
  ...
  for (const colItem of readobj.columns) {
    ...
    try {
      if (res[dn_prop] && res[dn_prop].points.length) {
        ...
        if (readobj.edge) {
          await getNearbyPoints(leftPoint, step, q1, item.points);
          plugin.log('INFO: After getNearbyPoints for right: item.points=' + util.inspect(item.points, null, 4));
        }
      } else if (readobj.edge) {
        item.points = await getNearbyPointsWhenNodata(leftPoint, step, q1);
  ...
  async function getNearbyPoints(leftPoint, step, q1, points) {
    // добавим краевые точки
    const first = leftPoint ? getFirst(step, leftPoint, points) : '';
    if (first) points.unshift(first);

    let rightPoint = step ? '' : await queryNearbyPointFromDB({ ...q1, start: query.end, end: '' }, 'right');
    let last = getLast(step, rightPoint, points);
    if (last) points.push(last);
  }

  async function getNearbyPointsWhenNodata(leftPoint, step, q1) {
    ...
    return [
      { ...chartutil.getPointAtX(leftPoint, rightPoint, query.start), hide: 1 },
      { ...chartutil.getPointAtX(leftPoint, rightPoint, query.end), hide: 1 }
    ];
  }

  function getFirst(step, leftPoint, pointArr) {
    let first = leftPoint; // для step
    if (!step) {
      first = chartutil.getPointAtX(leftPoint, pointArr[0], query.start);
    }
  ...
  function getLast(step, rightPoint, pointArr) {
    ...
    if (step) {
      ...
      last = chartutil.getPointAtX(lastP, rightPoint, query.end);
    }
    return { ...last, hide: 1 };
  }

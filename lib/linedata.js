...
module.exports = async function(records, readobj, query, useIds, client, plugin) {
  ...
  for (const colItem of readobj.columns) {
    ...
    let q1 = { dn_prop, notnull: readobj.notnull };
    if (useIds) {
      try {
        const ids = getIdForDn_prop(dn_prop, query);
        if (!ids) throw { message: 'Expected not empty ids: ' + ids };
        if (typeof ids != 'string') throw { message: 'Expected string ids: ' + ids };
        q1 = { dn_prop, notnull: readobj.notnull, ids };
      } catch (e) {
        plugin.log('ERROR: catch getIdForDn_prop dn_prop=' + dn_prop + ' query=' + util.inspect(query) + util.inspect(e));
      }
    } else {
      q1 = { dn_prop, notnull: readobj.notnull };
    }
  ...
  async function queryNearbyPointFromDB(q1, side) {
    ...
    if (!sqlStr) {
      plugin.log('ERROR: queryNearbyPointFromDB sqlStr for query ' + util.inspect(q1) + ' is empty');
      return;
    }
    ...
    try {
      ...
      plugin.log('ERROR: linedata Query rejected for SQL: ' + sqlStr);
    }
  }
